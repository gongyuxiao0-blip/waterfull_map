...
def load_province_rainfall(province_name: str) -> pd.DataFrame:
    ...
    if df.empty:
        raise ValueError(f"未找到省份 {province_name} 的降雨数据。")
    return df
...
# 13. 主流程
# =========================
def train_and_predict(target_type: str, target_name: str, show_plot: bool = True):
    paths = get_model_paths(target_type, target_name)
    ...
    if show_plot:
        ...
        plt.plot(prepared.test_dates, prepared.y_test_mm, label="真实降雨量(mm)", alpha=0.7)
        plt.plot(prepared.test_dates, y_final, label="预测降雨量(mm)", alpha=0.7)
        plt.title(f"{target_name} 降雨量预测对比")
        plt.xlabel("日期")
        plt.ylabel("降雨量(mm)")
        plt.legend()
